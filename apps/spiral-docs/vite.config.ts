...
function writeLocalSpiralTailwindSources(_local: LocalSpiralPaths | null) {
  // Spiral residual utilities ship inside `@aviala-design/spiral/styles.css`.
  // Docs chrome still uses Tailwind for its own layout; no @source of spiral.
  const outDir = path.join(dirname, "src/.generated");
  const outFile = path.join(outDir, "local-spiral-sources.css");
  mkdirSync(outDir, { recursive: true });
  writeFileSync(outFile, "/* local Spiral Tailwind @source: off (styles prebundled) */\n");
}
